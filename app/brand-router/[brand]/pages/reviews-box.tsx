...
export function ReviewsBox({ businessId, initialReviews, avg, count, primary, secondary, accent, brandName, canReply, replyPhone, replyToken }: Props) {
  ...
  if (step === "done") {
    return (
      <div className="rounded-2xl border bg-white p-6 text-center shadow-sm" style={{ borderColor: `${accent}30` }}>
        <div className="text-4xl mb-2">✅</div>
        <h3 className="font-bold" style={{ color: primary }}>Review published</h3>
        <p className="text-sm opacity-60 mt-1">Thanks — your review helps others in {brandName}.</p>
        <button onClick={() => setStep("view")} className="press mt-4 text-sm font-semibold underline" style={{ color: primary }}>Back to reviews</button>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border bg-white p-6 shadow-sm" style={{ borderColor: `${accent}30` }}>
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-bold flex items-center gap-2" style={{ color: primary }}>
        ...
        {count > 0 && (
          <span className="text-sm font-semibold text-amber-600">
            ★ {avg?.toFixed(1)} <span className="font-normal text-neutral-400">({count})</span>
          </span>
        )}
      </div>
        ...
        <>
          {reviews.length === 0 ? (
            ...
            <div className="space-y-4 max-h-80 overflow-y-auto pr-1">
              {reviews.map((r) => (
                <div key={r.id} className="border-t pt-3" style={{ borderColor: `${accent}20` }}>
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-semibold">{r.author_name}</p>
                    <span className="text-xs text-amber-500 font-semibold">★ {r.rating}</span>
                  </div>
        ...
        <form onSubmit={sendOtp} className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {[1, 2, 3, 4, 5].map((n) => (
              <button type="button" key={n} onClick={() => setRating(n)}
                      className="text-xl leading-none" style={{ color: n <= rating ? "#f59e0b" : "#d4d4d8" }} aria-label={`${n} star`}>★</button>
            ))}
          </div>
          ...
          <div className="flex gap-2">
            ...
            <button type="submit" disabled={busy} className="flex-1 rounded-xl px-4 py-2.5 text-sm font-bold text-white shadow disabled:opacity-50"
                    style={{ background: `linear-gradient(135deg, ${primary}, ${secondary})` }}>{busy ? "..." : "Continue"}</button>
          </div>
        </form>
      )}
      ...
      {step === "otp" && (
        <form onSubmit={verifyAndPost} className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button type="submit" disabled={busy} className="flex-1 rounded-xl px-4 py-2.5 text-sm font-bold text-white shadow disabled:opacity-50"
                    style={{ background: `linear-gradient(135deg, ${primary}, ${secondary})` }}>{busy ? "..." : "Publish review"}</button>
          </div>
        </form>
      )}
    </div>
