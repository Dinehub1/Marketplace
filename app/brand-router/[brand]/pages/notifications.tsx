    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: bg }}>
      ...
      <main className="flex-1 mx-auto max-w-2xl px-6 py-8 w-full">
        ...
        <div className="flex gap-2 mb-6">
          {["all", "unread"].map((f) => (
            <button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${filter === f ? 'text-white shadow-sm' : 'bg-white border hover:shadow-sm'}`} style={filter === f ? { background: `linear-gradient(135deg, ${primary}, ${secondary})` } : { borderColor: `${accent}20`, color: primary }}>
        ...
        <div className="space-y-3">
          {filtered.map((n, i) => (
            <div key={i} className={`group card-lift rounded-2xl border bg-white p-4 flex items-start gap-4 ${!n.read ? 'border-l-4' : ''}`} style={{ borderColor: n.read ? `${accent}20` : undefined, borderLeftColor: !n.read ? primary : undefined }}>
              <span className="text-2xl flex-shrink-0 group-hover:scale-110 transition-transform">{n.icon}</span>
              <div className="flex-1 min-w-0">
                <p className="font-bold text-sm" style={{ color: primary }}>{n.title}</p>
                <p className="text-sm opacity-60 mt-0.5 truncate">{n.msg}</p>
                <p className="text-xs opacity-30 mt-1">{n.time}</p>
              </div>
              {!n.read && <div className="w-2.5 h-2.5 rounded-full mt-2 flex-shrink-0 animate-pulse" style={{ backgroundColor: primary }} />}
            </div>
          ))}
        </div>
