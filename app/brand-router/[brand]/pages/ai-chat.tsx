    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: bg }}>
      ...
      <main className="flex-1 flex flex-col mx-auto max-w-2xl w-full px-6 py-8">
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center gap-2 rounded-full border px-4 py-1.5 text-xs font-medium mb-3" style={{ borderColor: `${accent}50`, color: primary }}>
            <span className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: primary }} />
            Powered by AI
          </div>
          <h1 className="text-2xl font-bold" style={{ color: primary }}>AI Assistant</h1>
          <p className="text-sm opacity-60">Ask me anything about {brand.name}</p>
        </div>

        {/* Chat window */}
        <div className="flex-1 rounded-3xl border bg-white shadow-sm flex flex-col overflow-hidden" style={{ borderColor: `${accent}20`, minHeight: "400px" }}>
          {/* Messages */}
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"} animate-scale-in`}>
                <div className="flex items-end gap-2 max-w-[80%]">
                  {m.role === "ai" && (
                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{ background: `linear-gradient(135deg, ${primary}, ${secondary})` }}>
                      AI
                    </div>
                  )}
                  <div className="rounded-2xl px-4 py-3 text-sm leading-relaxed" style={m.role === "user" ? { background: `linear-gradient(135deg, ${primary}, ${secondary})`, color: "white", borderBottomRightRadius: "4px" } : { backgroundColor: `${primary}08`, borderBottomLeftRadius: "4px" }}>
                    {m.text}
                  </div>
                </div>
              </div>
            ...
            {typing && (
              <div className="flex justify-start animate-scale-in">
                <div className="flex items-end gap-2">
                  <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: `linear-gradient(135deg, ${primary}, ${secondary})` }}>AI</div>
                  <div className="rounded-2xl px-4 py-3 flex gap-1" style={{ backgroundColor: `${primary}08` }}>
          ...
          <div className="px-5 pb-2 flex gap-2 overflow-x-auto">
            {["Pricing info", "Book appointment", "Services offered", "Contact details"].map((q) => (
              <button key={q} onClick={() => { setInput(q); }} className="flex-shrink-0 text-xs px-3 py-1.5 rounded-full border transition-colors hover:bg-gray-50" style={{ borderColor: `${accent}30`, color: primary }}>
                {q}
              </button>
            ))}
          </div>
          ...
          {/* Input */}
          <div className="border-t p-4 flex gap-3" style={{ borderColor: `${accent}15` }}>
            <input
              value={input}
            ...
            />
            <button onClick={sendMessage} className="rounded-xl px-5 py-3 text-white text-sm font-bold hover:shadow-md transition-all hover:translate-y-[-1px]" style={{ background: `linear-gradient(135deg, ${primary}, ${secondary})` }}>
              Send
            </button>
          </div>
        </div>
