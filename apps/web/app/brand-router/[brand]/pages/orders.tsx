    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 mx-auto max-w-3xl px-6 py-8 w-full">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold" style={{ color: "var(--brand-secondary)" }}>My Orders</h1>
        ...
        <div className="space-y-4">
          {ORDERS.map((o) => (
            <div key={o.id} className="card-lift rounded-2xl border bg-surface p-5 shadow-sm" style={{ borderColor: "var(--hairline)" }}>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <p className="font-bold text-base" style={{ color: "var(--brand-secondary)" }}>{o.item}</p>
                  <p className="text-xs opacity-40 mt-0.5">{o.id} · {o.date}</p>
                </div>
                <div className="text-right">
                  <p className="font-bold text-xl" style={{ color: "var(--brand-secondary)" }}>{o.amount}</p>
                  <StatusBadge status={o.status} />
                </div>
              </div>

              {/* Timeline */}
              <div className="flex items-center gap-1 mt-4 pt-4 border-t" style={{ borderColor: "var(--hairline)" }}>
                {["Placed", "Confirmed", "Shipped", "Delivered"].map((step, i) => (
                  <div key={step} className="flex-1">
                    <div className="w-full h-2 rounded-full overflow-hidden bg-surface-sunken">
                      <div className="h-full rounded-full transition-all duration-700" style={{ width: o.timeline[i] ? "100%" : "0%", background: "var(--brand-primary)" }} />
                    </div>
                  </div>
                ))}
              </div>
