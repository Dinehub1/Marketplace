      ...
      <main className="flex-1">
        <section className="border-b border-black/[0.06]">
          <div className="mx-auto max-w-5xl px-6 pt-14 pb-10">
            <nav className="mb-4 flex flex-wrap items-center gap-1.5 text-xs text-ink-3">
              <a href={origin} className="press hover:text-ink-2">{brand.name}</a>
              <span aria-hidden="true">›</span>
              <a href={`${origin}/marketplace`} className="press hover:text-ink-2">Directory</a>
              <span aria-hidden="true">›</span>
              <span className="text-ink-3">All categories</span>
            </nav>
            ...
            <div className="flex gap-1">
              {letters.map((l) => (
                <a key={l} href={`#letter-${l}`}
                   className="press flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg text-xs font-bold text-ink-3 hover:bg-surface-sunken">
                  {l}
                </a>
              ))}
            </div>
        ...
        <section className="mx-auto max-w-5xl px-6 py-10">
          {letters.map((l) => (
            <div key={l} id={`letter-${l}`} className="mb-10 scroll-mt-28">
              <h2 className="mb-3 flex items-center gap-3 text-sm font-bold text-ink-3">
                <span>{l}</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5">
                {groups.get(l)!.map((c) => (
                  <a key={c.slug} href={`${origin}${categoryPath(c.category)}`}
                     className="press group flex items-center gap-2.5 rounded-xl px-3 py-2.5 hover:bg-surface hover:ring-1 hover:ring-black/[0.05]">
                    <span className="text-ink-4 group-hover:text-current transition-colors"
