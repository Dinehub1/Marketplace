    ...
    <div className="flex min-h-screen items-center justify-center px-4" style={{ backgroundColor: bg }}>
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-3xl font-bold" style={{ color: "var(--brand-secondary)" }}>{brand.name}</h1>
        ...
        <div className="rounded-2xl border bg-surface p-8 shadow-sm">
          {step === "phone" ? (
            <form onSubmit={sendOtp} className="space-y-4">
              <h2 className="text-lg font-semibold" style={{ color: "var(--brand-secondary)" }}>Sign in with WhatsApp</h2>
              ...
              <div>
                ...
                <div className="flex items-center rounded-lg border border-hairline focus-within:border-[var(--brand-secondary)]">
                  <span className="px-3 text-sm text-ink-3">+91</span>
                  <input
                    type="tel"
                    inputMode="numeric"
                    autoFocus
              ...
              {error && <p className="rounded-lg bg-[var(--critical-tint)] p-2 text-xs tone-critical">{error}</p>}
              <button
                type="submit"
                disabled={loading}
            ...
            <form onSubmit={verifyOtp} className="space-y-4">
              <h2 className="text-lg font-semibold" style={{ color: "var(--brand-secondary)" }}>Enter the code</h2>
              <p className="text-xs opacity-60">
                Sent to <span className="font-medium">{phone}</span>{" "}
                <button type="button" onClick={() => { setStep("phone"); setCode(""); setError(null); setDevOtp(null); }} className="underline" style={{ color: "var(--brand-secondary)" }}>change</button>
              </p>
              {isDev && devOtp && (
                <div className="rounded-lg border border-hairline bg-[var(--gold-tint)] p-3 text-center">
                  <p className="text-[11px] font-semibold uppercase tracking-wide tone-gold">DEV OTP — WhatsApp may not deliver in dev</p>
                  <p className="mt-1 text-3xl font-bold tracking-[0.5em] tone-gold">{devOtp}</p>
                </div>
              )}
              <input
                type="text"
                ...
                className="w-full rounded-lg border border-hairline px-3 py-3 text-center text-2xl tracking-[0.5em] outline-none focus:border-[var(--brand-secondary)]"
                placeholder="••••••"
              />
              {error && <p className="rounded-lg bg-[var(--critical-tint)] p-2 text-xs tone-critical">{error}</p>}
              <button
